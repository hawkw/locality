...
// Local is always Sync, even if T isn't
...
impl<T: Send> Local<T> {
    ...
    fn insert(&self, id: usize, data: Box<T>, new: bool) -> &T {
        ...
        for entry in table.entries.iter().cycle().skip(hash(id, table.hash_bits)) {
            ...
            if owner == id {
                ...
                // Local between our calls to get_fast() and insert(). We
    ...
    ///
    /// Since this call borrows the `Local` mutably, this operation can
    ...
    /// Removes all thread-specific values from the `Local`, effectively
...
impl<T: Send> IntoIterator for Local<T> {
    type Item = T;
...
impl<'a, T: Send + 'a> IntoIterator for &'a mut Local<T> {
    type Item = &'a mut T;
...
impl<T: Send + fmt::Debug> fmt::Debug for Local<T> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "Local {{ local_data: {:?} }}", self.get())
    }
}

impl<T: Send + UnwindSafe> UnwindSafe for Local<T> {}
...
/// Mutable iterator over the contents of a `Local`.
pub struct IterMut<'a, T: Send + 'a> {
    raw: RawIter<T>,
    marker: PhantomData<&'a mut Local<T>>,
}
...
/// An iterator that moves out of a `Local`.
pub struct IntoIter<T: Send> {
    raw: RawIter<T>,
    _thread_local: Local<T>,
}
...
mod tests {
    ...
    fn same_thread() {
        ...
        let mut tls = Local::new();
        assert_eq!(None, dbg!(tls.get()));
        assert_eq!("Local { local_data: None }", format!("{:?}", &tls));
        ...
        assert_eq!(0, *tls.get_or(|| create()));
        assert_eq!(Some(&0), tls.get());
        assert_eq!("Local { local_data: Some(0) }", format!("{:?}", &tls));
        tls.clear();
    ...
    //     let create = make_create();
    //     let tls = Arc::new(CachedLocal::new());
    //     assert_eq!(None, tls.get());
    //     assert_eq!(0, *tls.get_or(|| create()));
    //     assert_eq!(Some(&0), tls.get());
    ...
    fn is_sync() {
        fn foo<T: Sync>() {}
        foo::<Local<String>>();
        foo::<Local<RefCell<String>>>();
        // foo::<CachedLocal<String>>();
        // foo::<CachedLocal<RefCell<String>>>();
    }
}
